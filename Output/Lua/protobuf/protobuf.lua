...
local function make_descriptor(name, descriptor, usable_key)
    ...
    meta.GetDescriptor = function() return descriptor end
    meta.__index = meta
...
local function _AddPropertiesForFields(descriptor, message_meta)
    for _, field in ipairs(descriptor.fields) do
        ...
        message_meta._field_descriptors[field.name] = field
    end
...
local function _AddStaticMethods(descriptor, message_meta)
    ...
    message_meta._member.GetDescriptor = function ()
        return descriptor
    end
    
    message_meta._member.GetFieldDescriptor = function (field_name)
        return message_meta._field_descriptors[field_name]
    end
end
...
local function _AddGetMessageMethod(message_meta, message_class)
    message_meta._member.GetMessage = function (self)
...
local function _AddMessageMethods(message_descriptor, message_meta, message_class)
    _AddGetMessageMethod(message_meta, message_class)
...
local function Message(descriptor)
    ...
    message_meta._field_descriptors = {}   -- name => field_desciptor
--    message_meta._name = descriptor.full_name
    ...
    message_meta._member.__tostring = function() return descriptor.full_name end
    ...
    _AddStaticMethods(descriptor, message_meta)
    _AddMessageMethods(descriptor, message_meta, ns)
